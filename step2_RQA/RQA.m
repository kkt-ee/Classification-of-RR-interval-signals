...
%% core fUNCTION : RQA()
function RQAstatistics=RQA(sig,filename)%,resultdir)
global resultdir
%% 1. Mutual Information Test to determine the *Time Delay*                           |tau|

mutual(sig,filename);%,resultdir);                                % fn call for plot 
mi=mutual(sig,filename);%,resultdir);                             % fn call for 'mi' values
for i=2:length(mi)-1
...
cerecurr_y(Y,filename);%,resultdir);
...
tdrecurr_y(cbuffer,0.3,filename);%,resultdir); %DEFAULT=0.3
